'''
    힙(Heap)
        ...
        힙을 사용하는 이유 
            ...
            이에반해, 힙에 데이터를 넣고, 최대값 최소값을 찾으면, O(logn)이 걸림
    ...
    힙의 구조
        ...
        힙은 다음과 같이 두 가지 조건을 가지고 있는 자료구조임 
            1. 각 노드의 값은 해당 노드의 자식 노드가 가진 값도가 크거나 같다. (최대 힘의 경우)
                최소 힙의 경우는 각 노드의 값은 해당 노드의 자식 노드가 가진 값도가 크거나 작음 
            2. 완전 이진 트리 형태를 가짐
    ...
    힙과 이진 탐색 트리의 공통점과 차이점 
        ...
        차이점
            ...
            힙은 이진 탐색 트리의 조건인 자식 노드에서 작은값은 왼쪽, 큰 값은 오른쪽이라는 조건은 없음
                힙의 왼쪽 및 오른쪽 자식 ㅗ드의 값은 오른쪽이 클 수도 있고, 왼쪽이 클 수도 있음
